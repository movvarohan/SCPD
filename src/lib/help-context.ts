...
=== WHAT THE APP IS ===
The SC Sourcing Engine automates client sourcing end to end: find leads, dedupe, enrich emails, research companies, draft outreach with AI, SEND automatically from your own Gmail, run follow-ups, track replies, and assign booked calls to PDs (Project Directors). It is AUTOMATED BY DEFAULT — generated drafts send immediately, and Autopilot drafts+sends for top leads daily — with a one-click kill switch and a daily cap. The Review Queue is an exception queue (only drafts that couldn't auto-send, e.g. no email address). You can switch to manual approval by turning Automatic sending OFF in Settings.
...
- Forgot password: an admin issues a reset link from Settings -> Team & invites (click the key icon next to a member). The link is single-use, valid 24h, opens /reset/<token>, and signs the person in after they set a new password (all other sessions are logged out). After 8 failed sign-ins an account is locked for 15 minutes.
...
REVIEW QUEUE (/review): the EXCEPTION queue. With automation on, only drafts that couldn't auto-send land here (no email address, rule mismatch, or a send failure). Each shows full context — why the lead was selected, personalization source, confidence, warnings. Actions: edit, Approve, Approve & send, Mark ready, Regenerate, Reject. With automatic sending turned off, every draft comes here for manual approval.
...
SETTINGS (/settings): organization name/description, default signature, ALLOWED CLAIMS (the AI may never exceed these — don't add client names unless approved), target industries, CAN-SPAM mailing address + footer toggle, email templates, Team & invites (with password-reset links), Auto-send + Autopilot rules, an Audit log (admins: sign-ins, invites, role changes, automation, key changes), and the API keys & mailbox panel. Press Cmd/Ctrl+K anywhere to open a command palette that jumps to any page or searches leads.

=== AUTO-SEND & FOLLOW-UPS ===
- Automatic sending (Settings → Auto-send rules, ON by default): generated drafts send immediately when they match the rule (industries / company sizes / seniorities / minimum score; blank = any) and pass the optional guardrails (verified-email-only, skip-if-warnings) within a daily cap. Turning this OFF routes everything to the Review Queue for manual approval. Autopilot (also in that panel, on by default): a daily job picks the top-scored uncontacted leads, researches them, writes and sends the email — fully hands-off — up to the configured leads/day, only when a real mailbox is connected. The dashboard banner shows today's count and a one-click 'Pause' that disables both. Every send is logged on the lead's timeline and in the audit log.
